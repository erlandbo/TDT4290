...
class Trainer:
    ...
    def __init__(
        ...
        self.loss_criterion2 = nn.L1Loss()
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            ...
            for x_batch, y_batch, _ in self.train_data:
                ...
                y_batch1 = y_batch[:, 0].unsqueeze(1) # prob
                y_batch2 = y_batch[:, 1:] # reg
                #print(x_batch)
                ...
                #print(x_batch)
                predictions1, predictions2 = self.model.forward(x_batch)
                # Compute loss
                #print(y_batch2, predictions2)
                loss1 = self.loss_criterion(predictions1, y_batch1)
                loss2 = self.loss_criterion2(predictions2, y_batch2) / 10
    ...
    def compute_val_L1loss(self, dataloader, plot_curves=False):
        """
        Computes the mean Average Precision for a given dataloader
        for a multilabel classifier.
        """
        ...
        with torch.no_grad():
            for x_batch, y_batch, _ in dataloader:
                ...
                y_batch = to_cuda(y_batch)
                y_batch = y_batch[:, 1:]
                _, outputs = self.model(x_batch)
                total_batches += 1
                total_loss += self.loss_criterion2(outputs, y_batch)
                #output_probs = self.activation(outputs)
                #output_probs = output_probs.cpu()
                outputs = outputs.cpu()
                y_batch = y_batch.cpu()
    ...
    def validation_epoch(self, plot_pr_curves=False):
        ...
        average_l1loss = self.compute_val_L1loss(
            self.val_data,
            plot_curves=plot_pr_curves
        )
        ...
        print(
            f"Epoch: {self.epoch + 1}\n",
            f"Global step: {self.global_step}\n",
            f"Validation L1 Loss: {average_l1loss}\n",
            sep="\t"
        )
