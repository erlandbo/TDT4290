...
def parse_dataframe(datapath, class_):
    ...
    df["class"] = "True"
    df.to_csv("./test.csv")
    df = df[["entry", "exit", "class", "width", "height"]]
    return df
...
def split_to_dirs(datapath, audio_path, class_, name_audio, SR=16000):
    ...
    while audio_end < len_audio_sec:
        df = pd.DataFrame(columns=["onset", "offset", "class", "width", "height"])
        active_zone = False
        for j, row in data.iterrows():
            ...
            if contains_vehicle:
                ...
                df.loc[len(df)] = {
                    "onset": max(0, vehicle_start - audio_start - buffer), "offset": min(10, vehicle_end - audio_start + buffer), "class": class_, "width": width, "height": height
                }
                active_zone = True
...
if __name__ == "__main__":
    ...
    split_to_dirs(data, audio, class_, name_audio)
