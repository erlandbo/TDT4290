...
def train(model, train_loader, valid_loader, epochs, optimizer, train_losses, valid_losses, device, learning_rate, change_lr=None):
    ...
    for epoch in tqdm(range(1,epochs+1)):
        ...
        error = np.sqrt(np.mean((trace_yhat - trace_y)**2))
        print(f'Epoch - {epoch} Valid-Loss : {np.mean(valid_losses[-1])} Valid-RMSE : {error}')
...
def main(dim):
    ...
    resnet_model.fc = nn.Sequential(nn.Linear(512,512), nn.Linear(512,512), nn.Linear(512,1))  # (512, 50)
...
if __name__ =="__main__":
    ...
    parser.add_argument("-c", "--class", default="width", type=str, help="Dimension of vehicle in ANN model")
    args = vars(parser.parse_args())
    dim = args["class"]
    assert dim in ["height", "width", "front_area"], "Invalid Dimension"
    main(dim)